...
namespace Crimson
{
    ...
    public class Chooser<T>
    {
        ...
        public Chooser(params T[] choices)
            : this()
        {
            foreach (var choice in choices) Add(choice, 1);
        }
        ...
        public bool CanChoose => TotalWeight > 0;
        ...
        public T Choose()
        {
            ...
            var roll = Utils.Random.NextDouble() * TotalWeight;
            float check = 0;

            for (var i = 0; i < choices.Count - 1; i++)
            {
                check += choices[i].Weight;
                if (roll < check) return choices[i].Value;
            }
