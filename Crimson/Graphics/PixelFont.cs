...
namespace Crimson
{
    ...
    {
        private readonly StringBuilder temp = new StringBuilder();
        ...
        {
            if (Characters.TryGetValue(id, out var val)) return val;
        ...
        {
            if (Characters.TryGetValue(text, out var c)) return new Vector2(c.XAdvance, LineHeight);
        ...
        )
        {
            ...
            for (var i = 0; i < text.Length; i++)
            {
                if (text[i] == '\n')
                {
                    offset.X = 0;
                    offset.Y += LineHeight;
                    if (justify.X != 0) justified.X = WidthToNextLine(text, i + 1) * justify.X;
