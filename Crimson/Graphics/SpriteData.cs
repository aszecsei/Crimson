    ...
    public class SpriteData
    {
        ...
        public void Add(XmlElement xml, string overridePath = null, string pathPrefix = null)
        {
            ...
            //Error Checking
            {
                ...
                foreach (XmlElement anim in source.XML.GetElementsByTagName("Anim")) CheckAnimXML(anim, prefix, ids);

                foreach (XmlElement loop in source.XML.GetElementsByTagName("Loop")) CheckAnimXML(loop, prefix, ids);
            ...
            //Create the Sprite
            {
                ...
                foreach (XmlElement anim in source.XML.GetElementsByTagName("Anim"))
                {
                    ...
                    var frames = Utils.ReadCSVIntWithTricks(anim.Attr("frames", ""));
                ...
                foreach (XmlElement loop in source.XML.GetElementsByTagName("Loop"))
                {
                    ...
                    var frames = Utils.ReadCSVIntWithTricks(loop.Attr("frames", ""));
                ...
                //Position
                if (source.XML.HasChild("Position")) Sprite.Position = source.XML.ChildPosition("Position");

                //Start Animation
                if (source.XML.HasAttr("start")) Sprite.Play(source.XML.Attr("start"));
            }
        ...
        {
            if (frames == null || frames.Length <= 0) return atlas.GetAtlasSubtexturesAt(path, 0) != null;
        ...
        {
            if (!xml.HasAttr("id")) throw new Exception(prefix + "'id' is missing on " + xml.Name + "!");
