...
namespace Crimson
{
    public class ParticleSystem : Entity
    {
        ...
        {
            for (var i = 0; i < amount; i++)
                Emit(type, Utils.Random.Range(position - positionRange, position + positionRange), color);
        }
        ...
        {
            for (var i = 0; i < amount; i++)
                Emit(type, Utils.Random.Range(position - positionRange, position + positionRange), color, direction);
        }
        ...
        )
        {
            for (var i = 0; i < amount; i++)
            {
                type.Create(ref particles[nextSlot], track,
                    Utils.Random.Range(position - positionRange, position + positionRange), direction,
                    type.Color);
                nextSlot = (nextSlot + 1) % particles.Length;
            }
        }
