...
namespace Crimson.Physics
{
    public class BoxCollider : Collider
    {
        ...
        public bool Intersects(BoxCollider hitbox)
        {
        ...
        {
            return new BoxCollider(width, height, Position.X, Position.Y);
        }
        ...
        {
            return Physics.Collide.RectToPoint(AbsoluteLeft, AbsoluteTop, Width, Height, point);
        }

        public override bool Collide(Rectangle rect)
        {
            return AbsoluteRight > rect.Left && AbsoluteBottom > rect.Top && AbsoluteLeft < rect.Right &&
                   AbsoluteTop < rect.Bottom;
        }

        public override bool Collide(Vector2 from, Vector2 to)
        {
            return Physics.Collide.RectToLine(AbsoluteLeft, AbsoluteTop, Width, Height, from, to);
        }
        ...
        {
            return Physics.Collide.RectToCircle(AbsoluteLeft, AbsoluteTop, Width, Height, circle.AbsolutePosition,
                                                circle.Radius);
        }
        ...
        public void Rotate()
        {
            float wasX = Position.X;
            float wasY = Position.Y;
            float wasHeight = Height;
            float wasWidth = Width;

            Height = wasWidth;
            Width = wasHeight;
            Position.X = wasY;
            Position.Y = -wasX - Height;
        }
    }
}
